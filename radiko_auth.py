...
APPVERSION_LIST = ['7.2.9','7.2.5','7.2.4','7.2.1','7.2.0','7.1.13','7.1.11','7.1.1','7.1.0','7.0.9','7.0.4','7.0.3','7.0.2','7.0.1','7.0.0','6.4.7','6.4.6'];



def genRandomInfo():

    #print(VERSION_MAP.keys())
    #print(list(VERSION_MAP.keys()))
    version_key = random.choice(list(VERSION_MAP.keys()))
    model = random.choice(MODEL_LIST)
    device = VERSION_MAP[version_key]['sdk'] + '.' + model
    build = random.choice(VERSION_MAP[version_key]['builds'])

    useragent = 'Dalvik/2.1.0 (Linux; U; Android ' + version_key + '; ' + model + '/' + build + ')';
    appversion = random.choice(APPVERSION_LIST)

    userid = ''
    for i in range(0, 32):
        userid += random.choice('0123456789abcdef')
...
def auth1(info):
    ...
    r = http.request('GET', 'https://radiko.jp/v2/api/auth1')
...
def genGPSstr(area_id):
    idx = int(area_id.replace('JP','')) - 1
    key = list(coordinates.keys())[idx]
    print('idx=%u, key=%s' % (idx, key))
    latitude = coordinates[key][0] + random.random() / 40.0 * (1 if random.random() > 0.5 else -1)
    longitude = coordinates[key][1] + random.random() / 40.0 * (1 if random.random() > 0.5 else -1)
    ret = '%.7f,%.7f,gps' % (latitude, longitude)
    print('%s -> %s' % (coordinates[key], ret))
    print('https://www.google.com/maps?q=%.7f,%.7f' % (latitude, longitude))

    return ret


def auth2(info, token, location, partial):
    headers = {
        'User-Agent': info['useragent'],
        'X-Radiko-App': 'aSmartPhone7a',
        'X-Radiko-App-Version': info['appversion'],
    ...
    r = http.request('GET', 'https://radiko.jp/v2/api/auth2')
    print('status=', r.status)
    print('headers=', r.headers)
    print('data=', r.data)
...
def readKey():
    key_file = os.getenv('radiko_auth_key', './radiko_key.txt')
    f = open(key_file, 'r')
    for l in f:
        L = l.split(' ')
        if L[0] == 'const' and L[1] == 'fullkey_b64' and L[2] == '=':
            key = L[3]
            break
    return key
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        print('uri_path=', sys.argv[1])
        area_id = uripath2areaid(sys.argv[1])
    ...
    #
    auth1_header = auth1(info)
    print(auth1_header)

    #
    token = auth1_header['X-RADIKO-AUTHTOKEN']
    offset = int(auth1_header['X-Radiko-KeyOffset'])
    length = int(auth1_header['X-Radiko-KeyLength'])
    print('token=%s, offset=%u, length=%u' % (token, offset, length))
    partial = base64.b64encode(base64.b64decode(fullkey_b64)[offset:offset+length])
    print('partial=%s' % partial)

    #
    location = genGPSstr(area_id)

    #
    auth2(info, token, location, partial)
    ...
    f = open(user_agent_file, 'w')
    f.write(useragent_str)
    f.close()
